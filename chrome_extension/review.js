...
async function trackFirstCheck() {
    ...
    if (!first_check_completed || shouldShowAfterClose) {
        console.log('Showing review after delay');
        await chrome.storage.local.set({ [STORAGE_KEYS.FIRST_CHECK]: true });
        setTimeout(showReviewRequest, 3000);
    } else {
        console.log('Not showing review - conditions not met');
    }
}
...
// Export as window properties since we can't use ES modules in Chrome extensions
window.trackFirstCheck = trackFirstCheck;
window.showReviewRequest = showReviewRequest;
window.CHROME_STORE_REVIEW_URL = CHROME_STORE_REVIEW_URL; 